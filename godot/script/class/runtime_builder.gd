...
func prepare_and_launch(game_name: String, mod_name: String) -> Util.Stats:
	...
	if(!FileAccess.file_exists(executable_path)):
		return Util.Stats.new(false, "error.prepared_executable_not_found")
	...
	if(pid == -1):
		return Util.Stats.new(false, "error.failed_to_launch_executable")

	return Util.Stats.new(true, "status.launched_game", {"pid": pid, "path": executable_path})
...
func _launch_with_steam(game_dir: String, run_dir: String, executable_path: String, steam_uri: String, steam_game_path: String) -> Util.Stats:
	if(steam_uri.is_empty() || steam_game_path.is_empty()):
		return Util.Stats.new(false, "error.steam_settings_pair_required")
	if(!DirAccess.dir_exists_absolute(steam_game_path)):
		return Util.Stats.new(false, "error.steam_game_path_not_found")
	...
	Filesys.delete_directory_if_exists(backup_dir)
	...
	return Util.Stats.new(true, "status.launched_game", {"path": steam_game_path, "steam_uri": steam_uri})

func _deploy_to_steam_game_path(run_dir: String, steam_game_path: String) -> Util.Stats:
	Filesys.delete_directory_contents(steam_game_path)
...
func _wait_for_process_exit(executable_name: String) -> Util.Stats:
	...
	for _i in range(120):
		...
		OS.delay_msec(1000)
	if(!saw_process):
		return Util.Stats.new(false, "error.steam_game_process_not_found")
